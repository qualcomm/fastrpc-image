...
The following example generates a *boot.img* for the Kailua MTP, as found in
...
## Generate Boot.img
One can also use it to build a boot image. The following example captures how to fetch and build
a boot image of the upstream Linux kernel for the Kailua MTP.
...
The resulting **boot.img** is ready to be booted on a Kailua MTP. But as the
